...
{
    public class BaseDB<TEntity> : IBaseDB<TEntity> where TEntity : class, new()
    {
        private SqlSugarClient _dbBase;
        public ISqlSugarClient _db => _dbBase;

        public BaseDB(IDbClient dbClient)
        {
            _dbBase = dbClient.GetDbClient();
        }
        ...
        {
            return await Task.Run(() => _db.Ado.SqlQueryAsync<dynamic>(strSql, parameters));
        }
